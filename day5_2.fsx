...
let rec FindGap (seat:int) (restOfSeats:int list) = 
    match restOfSeats with
    | nextSeat :: tail -> if nextSeat - seat = 2 then seat + 1 else (FindGap nextSeat tail)
    | _ -> 0
...
let answer = FindGap (List.head seatIds) (List.tail seatIds)
